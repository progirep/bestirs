#ifndef __GAME_SOLVER_BIG_BDD_SINGLE_ACTION_HPP
#define __GAME_SOLVER_BIG_BDD_SINGLE_ACTION_HPP

#include "gameSolver.hpp"
#include <limits>

template<int nofStateDimensions, int nofTranslationInvariantStateDimensions> class GameSolverBigBDDSingleActionComputation : public GameSolver<nofStateDimensions,nofTranslationInvariantStateDimensions> {
protected:
...
private:
    ...
    BF allActionsMoves;
    BF allActionsAllowed;

public:
    GameSolverBigBDDSingleActionComputation(TransitionFunction<nofStateDimensions,nofTranslationInvariantStateDimensions> &_transitionFunction, SettingReader &settingsReader, bool dynamicReodering, std::string &imposedVariableOrdering);
...
GameSolverBigBDDSingleActionComputation<nofStateDimensions,nofTranslationInvariantStateDimensions>::
GameSolverBigBDDSingleActionComputation
(TransitionFunction<nofStateDimensions,nofTranslationInvariantStateDimensions> &_transitionFunction, SettingReader &settingReader, bool dynamicReodering, std::string &imposedVariableOrdering) : GameSolver<nofStateDimensions,nofTranslationInvariantStateDimensions>(_transitionFunction,settingReader,dynamicReodering,imposedVariableOrdering) {
    ...
    allActionsMoves = mgr.constantFalse();
    allActionsAllowed = mgr.constantFalse();
    for (unsigned int i=0;i<actions.size();i++) {
        BF encode = encodeValueinBFs(bfActionVars,0,i);
        allActionsMoves |= actions[i].first & encode;
        allActionsAllowed |= actions[i].second & encode;
    }
...
template<int nofStateDimensions, int nofTranslationInvariantStateDimensions>
BF GameSolverBigBDDSingleActionComputation<nofStateDimensions,nofTranslationInvariantStateDimensions>::enfPre(BF targetStates) {

    BF targetStatesPost = targetStates.SwapVariables(allPreVarVector,allPostVarVector);
    BF allReachable = mgr.constantFalse();

    BF these = (targetStatesPost | !allActionsMoves).UnivAbstract(allPostVarCube) & allActionsAllowed;
    these = these.ExistAbstract(allActionCube);
    return these;
