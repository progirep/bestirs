...
int main(int argv, const char **args) {
  ...
  state_type s_lb={{0,0,-4}};
  /* upper bounds of the hyper rectangle */
  state_type s_ub={{10,10,4}};
  /* grid node distance diameter */
  state_type s_eta={{.2,.2,M_PI/32}};
  constexpr int nofRotations = 64;
      ...
      for(abs_type j=0; j<tf.m_no_inputs; j++) {
          for(abs_type v=0; v<tf.m_no_pre[k*tf.m_no_inputs+j]; v++) {
              ...
              if ((preGrid[0]==middleCell[0]) && (preGrid[1]==middleCell[1])) {
                  outFile << preGrid[0]-middleCell[0] << "," << preGrid[1]-middleCell[1] << "," << (preGrid[2]-middleCell[2]+10*nofRotations) % nofRotations << "," << j << ",";
                  outFile << postGrid[0]-middleCell[0] << "," << postGrid[1]-middleCell[1] << "," << (postGrid[2]-middleCell[2]+10*nofRotations) % nofRotations << "\n";
              }
          }
      }
  }
