...
#      Goals, and dynamics
# ==================================
CONFIGURATION_FILE_START = """nofDimensions=3
...
# XSIZE = 64
# YSIZE = 32
GOALS = [((0,6),(9,15)),((27,31),(0,6)) ]
BADREGIONS = [((6,11),(6,15)),((18,23),(0,6))]
XSIZE = 32
YSIZE = 16
...
# Continus DYNAMICS
STATE_ETA = (0.2,0.2,0.2)
CONTINUOUS_STATE = [0,0,0]
STATE_LB = (-0.5*STATE_ETA[0],-0.5*STATE_ETA[0],-3.5)
STATE_UB = ((XSIZE+0.5)*STATE_ETA[0],(YSIZE+0.5)*STATE_ETA[1],3.5)
INPUT_ETA = (0.3,0.3)
INPUT_LB = (-0.9-INPUT_ETA[0]/2.0,-0.9-INPUT_ETA[1]/2.0)
INPUT_UB = (0.9+INPUT_ETA[0]/2.0,0.9+INPUT_ETA[1]/2.0)
TIME_STEP = 0.5
def getGradient(x,u):
    xx = [0,0,0]
    alpha=math.atan(math.tan(u[1])/2.0);
    xx[0] = u[0]*math.cos(alpha+x[2])/math.cos(alpha);
    xx[1] = u[0]*math.sin(alpha+x[2])/math.cos(alpha);
    xx[2] = u[0]*math.tan(u[1]);
    return tuple(xx)
...
# Derivated values
# ==================================================
INPUT_NOF_VALUES_PER_DIMENSION = [ int(round(INPUT_UB[i]/INPUT_ETA[i]-0.5)-round(INPUT_LB[i]/INPUT_ETA[i]+0.5)+1) for i in range(0,len(INPUT_LB))]
INPUT_MIN_VALUE = [ round(INPUT_LB[i]/INPUT_ETA[i]+0.5)*INPUT_ETA[i] for i in range(0,len(INPUT_LB))]
STATE_MIN_VALUE = [ round(STATE_LB[i]/STATE_ETA[i]+0.5)*STATE_ETA[i] for i in range(0,len(STATE_LB))]
    ...
    def run(self):
        ...
        while True:
            ...
            for i in range(0,len(INPUT_LB)):
                ...
                continuousInput[i] = INPUT_MIN_VALUE[i]+localActionNum*INPUT_ETA[i]
            ...
            nextStates = [ [ int((CONTINUOUS_STATE[i]-STATE_MIN_VALUE[i])/STATE_ETA[i]) for i in range(0,len(STATE_LB)) ] ]
