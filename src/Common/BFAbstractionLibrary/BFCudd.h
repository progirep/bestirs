...
public:
    ...
    BFBdd compose(std::vector<std::pair<BFBdd,BFBdd> > const &substitutions) const {
        unsigned int allPart = bfmanager->getNofVars();
        DdNode *allSubstitute[allPart];
        for (unsigned int i=0;i<allPart;i++) allSubstitute[i] = Cudd_bddIthVar(mgr,i);
        for (auto it : substitutions) {
            allSubstitute[it.first.readNodeIndex()] = it.second.getCuddNode();
        }

        return BFBdd(bfmanager, Cudd_bddVectorCompose(mgr,node,allSubstitute));
    }
